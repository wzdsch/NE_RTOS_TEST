 ...
 * @LastEditTime: 2026-01-23 23:49:16
...
void GetFSI6Data(FSI6Data* fsi6Data,uint8_t* rxBuffer){
  ...
  fsi6Data ->left_x = ((uint16_t)rxBuffer[3] >> 6 | ((uint16_t)rxBuffer[4] << 2 ) | (uint16_t)rxBuffer[5] << 10 ) & 0x07FF;
  fsi6Data ->left_y = ((uint16_t)rxBuffer[5] >> 1 | ((uint16_t)rxBuffer[6] << 7 )) & 0x07FF;
...
void FSI6_BUS_IDLEHandler_Init(uint8_t *rx0_buf, uint8_t *rx1_buf, uint16_t dma_buf_num) {
    // 使能DMA串口接收功能
    ...
    // 设置DMA的内存缓冲区0的地址
    // DMA将接收到的数据存储到该缓冲区
    hdma_usart3_rx.Instance->M0AR = (uint32_t)(rx0_buf);
    // 设置DMA的内存缓冲区1的地址
    // DMA在两个缓冲区之间切换存储接收到的数据
    hdma_usart3_rx.Instance->M1AR = (uint32_t)(rx1_buf);
    // 设置DMA要传输的数据长度
    // 指定本次DMA传输的数据数量
    ...
    /*if(__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE)){
        ...
        if ((hdma_usart3_rx.Instance->CR & DMA_SxCR_CT) == RESET){
            ...
            fsi6_completed_buf = W_BusRxBuffer[1];
				...
				BaseType_t xLawPriorityTaskWoken = pdFALSE;
			  vTaskNotifyGiveFromISR(RemoteReceiveAnalysisHandle, &xLawPriorityTaskWoken);
				portYIELD_FROM_ISR(xLawPriorityTaskWoken);
        // 重新启动 DMA 接收
