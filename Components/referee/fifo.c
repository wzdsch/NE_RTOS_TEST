...
//! This function allocate enought room for N blocks fifo elements, then return
//! the pointer
//! of FIFO.
//!
...
//! \param  [in] p_base_addr is the base address of pre-allocate memory, such as
//! array.
//! \param  [in] uint_cnt is count of fifo elements.
...
char fifo_s_preread(fifo_s_t *p_fifo, int offset) {
  ...
  if (offset > p_fifo->used_num) {
    ...
    if (tmp_read_addr > p_fifo->p_end_addr)
      tmp_read_addr =
          tmp_read_addr - p_fifo->p_end_addr + p_fifo->p_start_addr - 1;
...
int fifo_pre_read(fifo_t *p_fifo, char offset, void *p_element) {
  ...
  while (_pre_red_index > p_fifo->p_end_addr) {
    _pre_red_index =
        _pre_red_index - p_fifo->p_end_addr + p_fifo->p_start_addr - 1;
  }
  //! Copy Data
...
int fifo_flush(fifo_t *p_fifo) {
  ...
  p_fifo->free_num =
      (p_fifo->p_end_addr - p_fifo->p_start_addr) / (p_fifo->unit_size);
  p_fifo->used_num = 0;
  p_fifo->p_read_addr = p_fifo->p_start_addr;
  p_fifo->p_write_addr = p_fifo->p_start_addr;
  // Interrupt On
