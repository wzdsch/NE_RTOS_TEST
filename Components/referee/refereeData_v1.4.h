...
typedef __packed struct {
  ...
  uint8_t BASE_VIRTUAL_STATE : 1;  // 己方基地虚拟护盾状态，1为基地有虚拟护盾血量
  uint8_t OUTPOST_STATE : 1;       // 己方前哨站状态，1为前哨站存活

  uint32_t UNKNOWN : 22;  // 预留
} event_data_t;
...
typedef __packed struct {
  ...
  uint16_t buffer_energy;    // 缓冲能量（单位：J）    (飞坡根据规则增加至250J)
 ...
 */
typedef __packed struct {
  uint8_t recovery_buff : 1;  // 机器人回血状态，1为有回血增益
  uint8_t cooling_buff : 1;   // 机器人枪口冷却加速状态，1为冷却加速
  uint8_t defence_buff : 1;   // 机器人防御增益状态，1为有防御增益
  uint16_t attack_buff : 1;   // 机器人攻击增益,1为有攻击增益
} robot_buff_t;
 ...
 */
typedef __packed struct {
  uint8_t time_remain;  // 空中机器人可攻击时间，单位s.30s递减至0
} air_support_data_t;
...
typedef __packed struct {
  ...
  // 组成的数值为装甲模块或测速模块的 ID编号；其他原因扣血时，该数值为 0
 ...
 */
typedef __packed struct {
  uint16_t projectile_allowance_17mm;  // 17mm 弹丸全队剩余可兑换数量
  uint16_t projectile_allowance_42mm;  // 42mm 弹丸全队剩余可兑换数量
...
typedef __packed struct {
  ...
  uint8_t hill_buff_area : 1;       // 高地增益点

  uint8_t self_energy_en_area : 1;  // 己方能量机关激活点
  uint8_t fly_buff_area : 1;        // 飞坡增益点

  uint8_t self_outpost_buff_area : 1;         // 己方前哨站增益点
  uint8_t self_medical_buff_area : 1;         // 己方补血点（检测到任一均视为激活）
  uint8_t self_engineering_rebirth_card : 1;  // 己方工程复活卡RFID状态

  uint32_t unknown : 24;  // 保留
} rfid_status_t;
 ...
 */
typedef __packed struct {
  uint8_t dart_launch_opening_status;  // 当前飞镖发射站的状态1:关闭，2：正在开启或关闭中，3：开启
  uint8_t dart_attack_target;  // 飞镖的打击目标：（默认为前哨站） 0:前哨站，1:基地
  uint16_t target_change_time;  // 切换打击目标时的比赛剩余时间，单位：s，无未切换动作默认为0。
  uint16_t
    latest_launch_cmd_time;  // 最后一次操作手确定发射指令时的比赛剩余时间，单位：s，初始值为0。
} dart_client_cmd_t;
...
typedef __packed struct {
  ...
  uint16_t
    receiver_id;  // 接收者ID,仅限己方通信,需为规则允许的车间通信接收者,若接收者为选手端，则仅可发送至发送者对应的选手端,ID编号详见附录
